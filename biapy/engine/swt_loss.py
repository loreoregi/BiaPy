"""
Stationary Wavelet Transform (SWT) loss, from the Wavelettention paper.

Decomposes prediction and target into wavelet subbands (LL/LH/HL/HH) and applies
an L1 loss per subband. Used as an additional term alongside the pixel loss for
SUPER_RESOLUTION (``LOSS.TYPE`` entry ``"SWT"``).

The SWT transform machinery (``SWTForward``/``SWTInverse`` and their helper
functions) is adapted from
https://github.com/qgpmztmf/Stationary_Wavelet_Transform_PyTorch
"""
import pywt
import numpy as np
import torch
...
def sfb1d_atrous(lo, hi, g0, g1, mode="symmetric", dim=-1, dilation=1, pad1=None, pad=None):
    ...
    y = F.conv_transpose2d(lo, g0, padding=unpad, groups=C, dilation=dilation) + F.conv_transpose2d(
        hi, g1, padding=unpad, groups=C, dilation=dilation
    )
...
class SWTLoss(nn.Module):
    """Stationary Wavelet Transform loss (subband L1), from the Wavelettention paper."""
    ...
    def __init__(
        ...
        # Built once in __init__, not on every forward call like the original.
        self.sfm = SWTForward(1, filters, "periodic").to(device)

    def _to_luma(self, x):
        assert x.shape[1] in (1, 3), f"SWTLoss expects 1 or 3 channels, got {x.shape[1]}"
        if x.shape[1] == 1:
            return x  # Grayscale: already the intensity signal.
    ...
    def forward(self, pred, target):
        ...
        return (
            self.loss_weight_ll * self.criterion(LL_sr, LL_hr)
            + self.loss_weight_lh * self.criterion(LH_sr, LH_hr)
            + self.loss_weight_hl * self.criterion(HL_sr, HL_hr)
            + self.loss_weight_hh * self.criterion(HH_sr, HH_hr)
        )
